...
handle(){
  this.setState({isLoading:true,isLight:! this.state.isLight,isDisabled:true});
}
...
render(){
  const {isLoading,islight,isDisabled} = this.state;
   const {icon,type,light,title,loader,disabler,changer,size,radius,style={}} = this.props;
   console.log(islight);
  return(
    <View>
  ...
  
{
  changer ? 
    (
<SocialIcon
  type={icon} button={true}  onPress= {this.handleLight} title={title} disabled={isDisabled} loading={isLoading}  
  light={islight} style={[styles.container,sizes[size],radius1[radius],style] }
/> ): <Text></Text>
        }
...
container:{
 ...
 padding: 20, height: 50,width:200
},
